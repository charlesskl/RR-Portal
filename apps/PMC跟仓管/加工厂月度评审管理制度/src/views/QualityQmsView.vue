...
const filtered = computed(() => records.value.filter(r => {
  ...
  return !dateError.value && (!factory.value || r.supplier === factory.value) &&
    (!from.value || date >= from.value) && (!to.value || date <= to.value) &&
    (!result.value || r.result === result.value) &&
    (!search.value.trim() || [r.supplier, r.productNo, r.productName, r.orderNo, r.defect, r.qc]
...
async function refresh() {
  ...
  try {
    ...
    if (!names.value.includes(factory.value)) factory.value = ''
  } catch (err: any) {
...
const columns = [
  ['inspDate', '验货日期'], ['companyName', '来源公司'], ['supplier', '加工厂 / 供应商'], ['client', '客户'], ['productNo', '货号'],
...
<template>
  <AppLayout>
    <div class="page">
      ...
      <p class="hint">数据来源：品质管理系统。按本厂区已有加工厂名称与供应商名称匹配，仅供查看。</p>
      <div class="filters">
        <label>加工厂<select v-model="factory"><option value="">全部加工厂</option><option v-for="name in names" :key="name">{{ name }}</option></select></label>
      ...
      <template v-else>
        <p class="hint">已有加工厂 {{ names.length }} 家 · 共 {{ filtered.length }} 条验货记录</p>
        <div class="table-scroll">
          <table>
            ...
            <tbody>
              ...
              <tr v-if="!rows.length"><td :colspan="columns.length" class="empty">{{ !names.length ? '该厂区暂无可查看的加工厂。' : '暂无匹配的验货记录，请检查筛选条件，以及加工厂名称是否与品质系统供应商名称一致。' }}</td></tr>
            </tbody>
          </table>
        </div>
