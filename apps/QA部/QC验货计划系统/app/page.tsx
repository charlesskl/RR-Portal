...
function Plans({query,setQuery,sessionUser,readOnly=false,batchId=null,onBatchChange}:{query:string;setQuery:(x:string)=>void;sessionUser:SessionUser|null;readOnly?:boolean;batchId?:number|null;onBatchChange?:(id:number|null)=>void}){
  ...
  const loadImported=useCallback(async (target:PlanSite,requestedMonth=selectedMonth,requestedPage=currentPage)=>{
    ...
    setLoadError('');
    setLoading(false);
    if(target===site){setImportedRows(result.items);setSelectedPlanIds([]);setMonths(result.months);setCustomers(result.customers);setLocations(result.locations);setSelectedMonth(result.month);setCurrentPage(result.page);setTotalPages(result.totalPages);setFilteredTotal(result.total);setLatestScheduleBatchId(result.latestScheduleBatchId??null)}
  ...
  return <List title="验货计划" sub="点击任意行即可直接编辑；修改内容统一保存。" action={readOnly?null:importActions}>
    ...
    {!readOnly&&Object.keys(inlineDrafts).length>0&&<div className="inline-save-bar"><span>已打开 {Object.keys(inlineDrafts).length} 条，待保存 {dirtyInlineIds.length} 条</span><small>浅绿色表示已修改字段；保存时整批校验，失败不会部分写入。</small><button onClick={discardInlineChanges} disabled={savingInline}>放弃全部修改</button><button className="save-all" onClick={()=>void saveInlineChanges()} disabled={dirtyInlineIds.length===0||savingInline}>{savingInline?'保存中…':`保存全部修改（${dirtyInlineIds.length}）`}</button></div>}
    {!readOnly&&<div className="bulk-delete-bar"><label><input type="checkbox" disabled={hasInlineChanges} checked={allVisibleSelected} onChange={toggleAllVisiblePlans}/> 全选本页可删除计划</label><span>已选 {selectedPlanIds.length} 条</span><button disabled={selectedPlanIds.length===0||bulkDeleting||hasInlineChanges} onClick={()=>void deleteSelectedInspections()}>{bulkDeleting?'正在删除…':`批量删除${selectedPlanIds.length?`（${selectedPlanIds.length}）`:''}`}</button></div>}
