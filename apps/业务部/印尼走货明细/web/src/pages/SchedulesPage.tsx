...
function DetailView({ detail }: { detail: ScheduleDetail }) {
  ...
  const keyOf = schedKeyOf
  const addedKeys   = useMemo(() => new Set(added.map(keyOf)), [added])
  const changedKeys = useMemo(() => new Set(changed.map(c => keyOf(c.to ?? c.from ?? {}))), [changed])
  // 每个改动行 → 变动关键字段的 原值/新值（数量/走货期/品名/单价），用于高亮单元格并 tooltip 显示差异
  const changedFields = useMemo(() => {
    const m = new Map<string, Record<string, { from: any; to: any }>>()
    for (const c of changed) {
      m.set(keyOf(c.to ?? c.from ?? {}), significantChangedDiff(c.from ?? {}, c.to ?? {}))
    }
    return m
  }, [changed])
  ...
  const placedKeyOf = (r: SchedRow) => `${r.orderNo ?? ''}|${r.code ?? ''}`
  // 切换手动标记并持久化
  ...
  // 已下单：自动(采购单命中) 或 手动标记
  const isAutoPlaced = (r: SchedRow) => !!(r.orderNo && r.code && placedKeys.has(placedKeyOf(r)))
  const isManualPlaced = (r: SchedRow) => manualKeys.has(placedKeyOf(r))
  const isPlaced = (r: SchedRow) => isAutoPlaced(r) || isManualPlaced(r)
  const isActualProductionPlaced = (r: SchedRow) => !!(r.orderNo && productionPlacedPos.has(r.orderNo.trim()))
  // 手动覆盖优先；未单独设置时生产状态跟随采购，真实生产单始终为已下。
  const isProductionPlaced = (r: SchedRow) => {
  ...
  const placedCount = useMemo(() => rawRows.filter(isPlaced).length, [rawRows, placedKeys, manualKeys])
  const productionPlacedCount = useMemo(() => rawRows.filter(isProductionPlaced).length, [rawRows, productionPlacedPos, productionManual, placedKeys, manualKeys])
