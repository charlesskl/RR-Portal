...
const profileFields: Array<{ name: keyof SupplierDict; label: string }> = [
  { name: 'keyword', label: '供应商简称（物料匹配关键字）' },
...
export default function SupplierSummaryPage() {
  ...
  const filtered = useMemo(() => rows.filter(r =>
    [r.keyword, r.full, r.nameEn, r.contact].some(x => (x || '').toLowerCase().includes(search.toLowerCase()))
  ), [rows, search])
  ...
  async function save() {
    ...
    setLoading(true)
    try {
      if (editing?.id) await api.put(`/dictionaries/suppliers/${editing.id}`, values)
      else await api.post('/dictionaries/suppliers', values)
  ...
  return <div style={{ padding: 16 }}>
    <Card title={`供应商汇总（${rows.length} 家）`} extra={<Space>
      <Input.Search allowClear placeholder="搜索简称、公司名或联系人" style={{ width: 300 }} onChange={e => setSearch(e.target.value)} />
      <Button onClick={load} loading={loading}>刷新</Button>
      <Button type="primary" disabled={!auth.canEdit('products')} onClick={add}>新增供应商</Button>
    </Space>}>
      <p style={{ color: '#666' }}>在此维护供应商简称及公司资料。物料明细中的卖方按对应供应商资料填入合同、发票和装箱单。</p>
      <Table rowKey={r => r.id || r.keyword} loading={loading} dataSource={filtered} scroll={{ x: 1500 }}
        pagination={{ defaultPageSize: 30 }} columns={[
          { title: '供应商简称', dataIndex: 'keyword', width: 160, fixed: 'left' },
          { title: '公司中文名称', dataIndex: 'full', width: 220 },
    ...
    <Modal title={editing ? `编辑供应商：${editing.keyword || ''}` : '新增供应商'} open={open} onCancel={() => setOpen(false)}
      ...
      <Form form={form} layout="vertical">
        {profileFields.map(f => <Form.Item key={f.name} name={f.name} label={f.label}
          rules={f.name === 'keyword' || f.name === 'full' ? [{ required: true, whitespace: true, message: '请填写' }] : f.name === 'email' ? [{ type: 'email', warningOnly: true }] : undefined}>
