...
// 数据依赖：dev.db 已 seed 管理员、兴信文员、华登文员
test.describe("P0 验收：登录 → Dashboard → 退出", () => {
  test("admin 完整流程", async ({ page }) => {
    ...
    await expect(page.getByRole("cell", { name: "huadeng_clerk" })).toBeVisible();
  ...
  test("华登文员看不到用户管理菜单", async ({ page }) => {
    await page.goto("/login");
    await page.getByLabel("用户名").fill("huadeng_clerk");
    await page.getByLabel("密码").fill("huadeng123");
    await page.getByRole("button", { name: /登录/ }).click();
    await expect(page).toHaveURL("http://localhost:8400/");
