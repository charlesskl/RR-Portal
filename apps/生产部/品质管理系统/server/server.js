/* ══════════════════════════════════════════════════════════════
   ...
   多厂区（2026-09 新增）：
     ...
     · 所有数据 API 通过 ?company=<厂区id> 指定目标厂区（缺省默认 dongguan，兼容旧链接）
     · 旧版单库 server/qc.db 首次启动自动迁移为东莞厂区库
...
const LEGACY_DB = path.join(__dirname, 'qc.db');        // 旧版单库路径（自动迁移用）
...
/* 旧版单库迁移：server/qc.db → data/dongguan/qc.db（仅首次） */
function migrateLegacyDb() {
  try {
    const dgDir = path.join(DATA_DIR, DEFAULT_COMPANY);
    const dgDb  = path.join(dgDir, 'qc.db');
    if (fs.existsSync(LEGACY_DB) && !fs.existsSync(dgDb)) {
      fs.mkdirSync(dgDir, { recursive: true });
      fs.renameSync(LEGACY_DB, dgDb);
      console.log('[迁移] 旧版数据库已迁移为东莞厂区库：', dgDb);
    }
  } catch (e) {
