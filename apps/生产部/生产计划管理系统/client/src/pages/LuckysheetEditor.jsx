    ...
    setTimeout(() => {
      try {
        ...
        const step = () => {
          ...
          else console.log('[公式恢复] 已恢复 ' + applied + ' 个公式单元格');
        };
        step();
      } catch (e) {
  ...
  function applyDaysFormulaBatch() {
    setTimeout(() => {
      try {
        ...
        const ls = getLuckysheet();
        // 早退也必须释放钩子抑制 —— 这是唯一的 suppress 释放点，
        // 卡住会导致用户所有编辑都记录不到、「保存不了」
        if (daysColIdx < 0 || qtyColIdx < 0 || targetColIdx < 0) { suppressHookRef.current = false; return; }
        if (!ls?.setCellValue) { suppressHookRef.current = false; return; }
        const sheet = ls.getAllSheets()[0];
        if (!sheet?.data) { suppressHookRef.current = false; return; }
        ...
        step();
      } catch (e) { console.warn('[天数自动算] 失败:', e?.message); suppressHookRef.current = false; }
    }, 600);
  }
