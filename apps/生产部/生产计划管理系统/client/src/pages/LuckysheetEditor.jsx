  ...
  orders.forEach((order, i) => {
    ...
    columns.forEach((col, c) => {
      ...
      if (isDateCol) {
        ...
        // 日期选择器靠 cellEditBefore hook + DATE_FIELDS 触发，跟 ct.t 无关。
      ...
      if (cellValue.v === '' && !cellFmt.bg && !cellFmt.fc && !cellFmt.bl && !cellFmt.f && !isNewImport && !isSumCol && !showRowColor) {
        return;
      }
...
const AUTO_BG_SET = new Set(['#E3F2FD', '#FFF9C4', '#FFFDE7', '#E6F4FF', '#D9F7BE', '#FFE7BA']);
...
function LuckysheetEditor({
  ...
  const recordCellChange = (r, c) => {
    try {
      ...
      let v = cell == null ? null : (typeof cell === 'object' ? (cell.v ?? null) : cell);
      if (v === '') v = null;
        ...
        setTimeout(() => {
          try {
            ...
            if (!isNaN(Mn) && !isNaN(ABn) && ABn !== 0 && M !== '' && M != null && AB !== '' && AB != null) {
              days = Math.round((Mn / ABn) * 10000) / 10000;   // 4 位小数
            }
            ls2.setCellValue(r, daysColIdx, days);
          } catch {}
        }, 50);
      }
    } catch (e) {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      if (updates.length === 0) {
        message.warning('没检测到任何改动（仅保存了布局）。按 F12 看控制台 [saveAll-双保险] 日志', 6);
      } else {
      ...
      try {
        ...
        for (let r = 1; r < sheet.data.length - 1; r++) {
          ...
          if (!isNaN(Mn) && !isNaN(ABn) && ABn !== 0 && M !== '' && M != null && AB !== '' && AB != null) {
            ...
            ls.setCellValue(r, daysColIdx, days);
            applied++;
          }
        }
