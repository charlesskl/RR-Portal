...
export async function POST(request: NextRequest) {
  try {
    const response = await fetch(`${backend}/api/imports/spreadsheet`, {
      method: "POST",
      headers: backendHeaders(request, { "content-type": request.headers.get("content-type") || "multipart/form-data" }),
