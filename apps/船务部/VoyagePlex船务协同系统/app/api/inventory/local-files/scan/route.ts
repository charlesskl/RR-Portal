...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${parser}/v1/local-inventory-files/scan`, { method:"POST", body:await request.formData() });
    return new NextResponse(await response.arrayBuffer(), { status:response.status,
      headers:{"content-type":response.headers.get("content-type")||"application/json"} });
  } catch (error) {
    return NextResponse.json({error:`库存读取服务连接失败：${error instanceof Error?error.message:"未知错误"}`},{status:502});
  }
}
