...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${backend}/api/shipments/completed/inventory-adjustment/export${request.nextUrl.search}`, { headers:backendHeaders(request), cache: "no-store" });
