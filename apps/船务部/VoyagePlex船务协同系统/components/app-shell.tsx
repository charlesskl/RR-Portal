...
type WarehouseGroup = { warehouse: string; references: string[]; items: Array<Record<string, unknown>>; source_files: string[] };
type ParsedEmail = {
...
export function AppShell({ route }: { route: string }) {
  ...
  async function loadUser(){setAuthLoading(true);try{const response=await fetch(apiPath("/api/auth/me"),{cache:"no-store"});if(response.ok){setUser(await response.json());setSetupRequired(false);}else{setUser(null);const setup=await fetch(apiPath("/api/auth/setup-status"),{cache:"no-store"});setSetupRequired(setup.ok&&Boolean((await setup.json()).required));}}finally{setAuthLoading(false);}}
...
function ImportCenter() {
  ...
  function updateEmailItem(itemIndex: number, key: string, value: string) {
    setEmailBatch(current => {
      ...
      const items = current.items.map((emailItem, emailIndex) => {
        ...
        return { ...emailItem, items: cargoItems };
      });
      return { ...current, items };
    });
  }
  ...
  return <div className="content">
    <div className="page-heading import-heading"><h1>信息导入与解析</h1></div>
    ...
    <section className="panel preview-panel"><div className="panel-title"><div><h2>解析结果预览</h2><p>{(email ? emailBatch : sheetBatch) ? `批次共 ${(email ? emailBatch : sheetBatch)!.total} 个：成功 ${(email ? emailBatch : sheetBatch)!.parsed}，失败 ${(email ? emailBatch : sheetBatch)!.failed}` : `选择${email ? "邮件" : "表格"}并解析后，在这里人工核对`}</p></div><b className={`status ${confirmed ? "ok" : (email ? emailBatch : sheetBatch)?.failed ? "danger" : "warn"}`}>{confirmed ? "已确认" : (email ? emailBatch : sheetBatch) ? "待确认" : "未解析"}</b></div>
      {email && emailBatch && <div className="email-review-layout">
        ...
        <div className="form-grid">{fields.map(([label,value], index) => { const key = ["so_number","container_type","ship_date","si_deadline","cutoff_date","port","destination_country","special_requirements"][index]; const deadline=key==="si_deadline"||key==="cutoff_date"; return <label key={label} className={key === "special_requirements" ? "full-width" : ""}><span>{label}</span>{key === "special_requirements" ? <textarea rows={4} value={String(value)} disabled={confirmed || selectedEmail?.status === "failed"} onChange={event => updateField(key, event.target.value)} /> : <input type={key === "ship_date" ? "date" : deadline ? "datetime-local" : "text"} value={key === "ship_date" ? dateControlValue(String(value)) : deadline ? dateControlValue(String(value),true) : String(value)} disabled={confirmed || selectedEmail?.status === "failed"} onChange={event => updateField(key, event.target.value)} />}</label>; })}</div>
          {!!selectedEmail?.warehouse_groups?.length && <div className="review-section"><h3>多仓分组</h3><div className="warehouse-grid">{selectedEmail.warehouse_groups.map((group,index) => <article key={`${group.warehouse}-${index}`}><label><span>仓库</span><input value={group.warehouse} disabled={confirmed} onChange={event => updateWarehouse(index,event.target.value)} /></label><small>SO/附件编号：{group.references.join("、")}</small><strong>{group.items.length} 条货物明细</strong></article>)}</div></div>}
          {!!selectedEmail?.items?.length && <div className="review-section"><h3>货物明细 <small>共{selectedEmail.items.length}条，可在确认前修改</small></h3><div className="review-items editable"><div><b>货号</b><b>货名/规格</b><b>合同号</b><b>客户PO</b><b>数量</b><b>件数</b><b>卡板数</b><b>体积(CBM)</b><b>工厂</b><b>长宽高(cm)</b></div>{selectedEmail.items.map((item,index) => { const fields = ["product_code","product_name","contract_number","customer_po","quantity","pieces","pallet_count","volume","supplier","box_dimensions"]; return <div key={index}>{fields.map(key => <input key={key} value={String(key === "product_name"?productNameSpec(item):(item[key] ?? ""))} disabled={confirmed} aria-label={`${key}-${index + 1}`} onChange={event => {if(key==="product_name"){const parsed=splitProductNameSpec(event.target.value);updateEmailItem(index,"product_name",parsed.product_name);updateEmailItem(index,"spec",parsed.spec);}else updateEmailItem(index,key,event.target.value);}} />)}</div>; })}</div></div>}
...
function ShipmentDetail({ id, role, directoryRole=role, backHref="/shipments", backLabel="返回走柜任务" }: { id:string; role:UserRole; directoryRole?:UserRole; backHref?:string; backLabel?:string }) {
  ...
  async function saveWarehouseQuantities() { if(!task)return; setSaving(true); setError(""); try { const response=await fetch(apiPath(`/api/shipments/${id}`),{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:task.items,transportReference:task.transportReference||""})}); const result=await response.json(); if(!response.ok) throw new Error(result.error||"保存失败"); setTask(result); setLocationNotice("柜号/车次、数量、件数、入库单号和放货区已保存"); } catch(reason){setError(reason instanceof Error?reason.message:"保存失败");} finally{setSaving(false);} }
  ...
  const itemFields=["product_code","product_name","spec","contract_number","customer_po","quantity","pieces","pallet_count","gross_weight","net_weight","volume","box_dimensions",...(warehouseUser?["inventory_receipt_number","warehouse_location","inventory_source"]:[])];
  ...
  return <div className="content shipment-detail"><Link className="back-button" href={backHref}><ArrowLeft size={15}/>{backLabel}</Link><div className="page-heading shipment-detail-heading"><div><p className="eyebrow">走柜任务 #{task.id}</p><h1 title={taskTitle}>{taskTitle}</h1></div><div className="heading-status-actions"><select className="heading-status-select" aria-label="任务状态" value={task.status} disabled={saving||warehouseUser||task.status!=="PendingReview"} onChange={event=>void update({status:event.target.value})}><option value={task.status}>{shipmentStatus[task.status]}</option>{task.status==="PendingReview"&&<option value="PendingShipment" disabled={!task.plannedShipDate}>{task.plannedShipDate?"待走货":"待走货（请先填写日期）"}</option>}</select>{!warehouseUser&&(task.status==="PendingReview"||task.status==="PendingShipment")&&<button className="ghost-button action-cancel" disabled={saving} onClick={()=>void cancelTask()}>取消任务</button>}<button className="ghost-button action-delete" disabled={saving} onClick={()=>void deleteTask()}><Trash2 size={14}/>删除任务</button></div></div>{error&&<div className="notice">{error}</div>}
    ...
    <section className={`panel shipment-items editable ${warehouseUser?"warehouse-view":"shipping-view"}`}><div className="panel-title"><div><h2>货物明细</h2><p>{warehouseUser?"放货区和入库单号从本地库存表查询，仓务可人工修正":"核对走柜货物资料、数量和装箱信息"}</p></div>{!warehouseUser&&<button className="ghost-button" onClick={addItem}><Plus size={14}/>新增明细</button>}</div><div className="shipment-item-table"><div><b>产品货号</b><b>货名</b><b>规格</b><b>合同号</b><b>客户PO</b><b>数量</b><b>件数</b><b>卡板</b><b>毛重</b><b>净重</b><b>CBM</b><b>尺寸</b>{warehouseUser&&<><b>入库单号</b><b>放货区</b><b>来源表</b></>}<b>操作</b></div>{task.items.map((item,index)=><div key={index}>{itemFields.map(key=>{const warehouseEditable=key==="quantity"||key==="pieces"||key==="inventory_receipt_number"||key==="warehouse_location";const warehouseLocked=warehouseUser&&(task.status!=="PendingShipment"||!warehouseEditable);const inventoryField=key==="inventory_receipt_number"||key==="warehouse_location";const pendingWeight=key==="gross_weight"||key==="net_weight";return <input key={key} className={inventoryField?"inventory-match-input":""} aria-label={`${key}-${index+1}`} title={key==="inventory_source"?String(item.inventory_source_full||item.inventory_source||""):undefined} disabled={warehouseLocked} placeholder={pendingWeight?"待货品资料":key==="inventory_receipt_number"?"待查询入库单":key==="warehouse_location"?"待查询放货区":key==="inventory_source"?"待查询来源表":""} value={key==="spec"?fullSpecification(item[key]):String(item[key]??"")} onChange={event=>changeItem(index,key,key==="spec"?specificationValue(event.target.value):event.target.value)}/>})}{warehouseUser?<span>{task.status==="PendingShipment"?"可保存":"只读"}</span>:<button className="icon-danger" aria-label={`删除第${index+1}条明细`} onClick={()=>removeItem(index)}><Trash2 size={14}/></button>}</div>)}</div></section>
  </div>;
}
