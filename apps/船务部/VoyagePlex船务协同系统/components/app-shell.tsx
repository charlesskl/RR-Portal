...
const apiFetch = (path: string, init?: RequestInit) => fetch(apiPath(path), init);
...
function EditableItemInput({value,disabled,label,placeholder,displayValue,onChange}:{value:unknown;disabled?:boolean;label:string;placeholder?:string;displayValue:(value:unknown)=>string;onChange:(value:string)=>void}){
  const [editing,setEditing]=useState(false);
  return <input aria-label={label} disabled={disabled} placeholder={placeholder} value={editing?String(value??""):displayValue(value)} onFocus={()=>setEditing(true)} onBlur={()=>setEditing(false)} onChange={event=>onChange(event.target.value)}/>;
}
...
function ShipmentDetail({ id, role, directoryRole=role, backHref="/shipments", backLabel="返回走柜任务" }: { id:string; role:UserRole; directoryRole?:UserRole; backHref?:string; backLabel?:string }) {
  const router=useRouter();
  const [task,setTask] = useState<ShipmentTaskData|null>(null); const [error,setError] = useState(""); const [saving,setSaving] = useState(false); const [qcNotice,setQcNotice] = useState(""); const [qcResults,setQcResults]=useState<QcResult[]|null>(null); const [qcCheckedAt,setQcCheckedAt]=useState(""); const [qcUnsaved,setQcUnsaved]=useState(false); const [queryingQc,setQueryingQc]=useState(false); const [locationNotice,setLocationNotice] = useState(""); const [refreshingLocations,setRefreshingLocations] = useState(false); const [writingBack,setWritingBack]=useState(false);
  useEffect(() => { setQcResults(null); setQcCheckedAt(""); setQcUnsaved(false); setQcNotice(""); void (async () => { try { const response=await apiFetch(`/api/shipments/${id}`,{cache:"no-store"}); const result=await response.json(); if(!response.ok) throw new Error(result.error||"读取任务失败"); const loaded=normalizeInventorySourceDisplay(result); const saved=readQcSnapshots(loaded.items); setTask(loaded); setQcResults(saved?.results||null); setQcCheckedAt(saved?.checkedAt||""); } catch(reason){setError(reason instanceof Error?reason.message:"读取任务失败");} })(); },[id]);
  async function update(changes:Record<string,string>) { if(!task)return; setSaving(true); setError(""); try { const response=await apiFetch(`/api/shipments/${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(changes)}); const result=await response.json(); if(!response.ok) throw new Error(result.error||"保存失败"); setTask(result); } catch(reason){setError(reason instanceof Error?reason.message:"保存失败");} finally{setSaving(false);} }
  async function cancelTask(){if(!task||!window.confirm("确认取消这个走柜任务？取消后不能恢复。"))return;await update({status:"Cancelled"});}
  async function deleteTask(){if(!task||!window.confirm(`是否删除任务“${task.customer} / ${task.soNumber||"SO待确认"}”及其邮件解析记录？\n\n删除后可重新导入原邮件；产品信息和产品映射不会删除。${task.items.some(item=>item.inventory_writeback_task_id)?"\n已写入本地库存的数据不会撤销。":""}`))return;setSaving(true);setError("");try{const response=await apiFetch(`/api/shipments/${id}`,{method:"DELETE"});const result=await readJsonResponse(response);if(!response.ok)throw new Error(result.error||"删除任务失败");router.push(backHref);router.refresh();}catch(reason){setError(reason instanceof Error?reason.message:"删除任务失败");setSaving(false);}}
  async function saveReview():Promise<boolean> { if(!task)return false; setSaving(true); setError(""); try { const items=qcResults&&qcCheckedAt?saveQcSnapshots(task.items,qcResults,qcCheckedAt):task.items; const response=await apiFetch(`/api/shipments/${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({customer:task.customer,soNumber:task.soNumber||"",containerType:task.containerType,plannedShipDate:task.plannedShipDate||"",cutoffDate:task.cutoffDate,siDeadline:task.siDeadline,port:task.port,destinationCountry:task.destinationCountry||"",transportReference:task.transportReference||"",specialRequirements:task.specialRequirements,warehouseGroups:task.warehouseGroups,items})}); const result=await readJsonResponse(response); if(!response.ok) throw new Error(result.error||"保存失败"); setTask(result); setQcUnsaved(false); return true; } catch(reason){setError(reason instanceof Error?reason.message:"保存失败");return false;} finally{setSaving(false);} }
  async function queryQcResult(){if(!task)return;setQueryingQc(true);setQcNotice("");try{const items=task.items.map(item=>({contractNumber:String(item.contract_number||""),customerPo:String(item.customer_po||""),itemNumber:String(item.product_code||"")}));const response=await apiFetch("/api/qc/results",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items})});const result=await readJsonResponse(response);if(!response.ok)throw new Error(result.error||"查询验货结果失败");setQcResults(result.results as QcResult[]);setQcCheckedAt(new Date().toISOString());setQcUnsaved(true);}catch(reason){setQcNotice(reason instanceof Error?reason.message:"查询验货结果失败");}finally{setQueryingQc(false);}}
  async function saveWarehouseQuantities():Promise<boolean> { if(!task)return false; setSaving(true); setError(""); try { const response=await apiFetch(`/api/shipments/${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:task.items,transportReference:task.transportReference||""})}); const result=await readJsonResponse(response); if(!response.ok) throw new Error(result.error||"保存失败"); setTask(result); setLocationNotice("柜号/车次、数量、件数、入库单号和放货区已保存"); return true; } catch(reason){setError(reason instanceof Error?reason.message:"保存失败");return false;} finally{setSaving(false);} }
  async function exportShipment(warehouse:boolean) {
    ...
    try{
      const response=await apiFetch(`/api/shipments/${id}/${warehouse?"warehouse-export":"export"}`,{cache:"no-store"});
  ...
  async function refreshWarehouseLocations() {
    ...
    try{
      ...
      const matchResponse=await apiFetch("/api/inventory/local-files/match",{method:"POST",body:form});const matchResult=await readJsonResponse(matchResponse);
      ...
      const saveResponse=await apiFetch(`/api/shipments/${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:matchedItems})});const saved=await readJsonResponse(saveResponse);
  ...
  function changeItem(index:number,key:string,value:string) { if(!task)return; const lookupChanged=key==="contract_number"||key==="customer_po"||key==="product_code"; if(lookupChanged){setQcResults(null);setQcCheckedAt("");setQcUnsaved(false);setQcNotice("");} setTask({...task,items:task.items.map((item,row)=>{if(row!==index)return item;const updated={...item,[key]:value};if(lookupChanged)delete updated.qc_snapshot;return updated;})}); }
  function removeItem(index:number) { if(task){setQcResults(null);setQcCheckedAt("");setQcUnsaved(false);setQcNotice("");setTask({...task,items:task.items.filter((_,row)=>row!==index)});} }
  function addItem() { if(task){setQcResults(null);setQcCheckedAt("");setQcUnsaved(false);setQcNotice("");setTask({...task,items:[...task.items,{product_code:"",product_name:"",spec:"",customer_po:"",contract_number:"",quantity:"",pieces:"",pallet_count:"",gross_weight:"",net_weight:"",volume:"",box_dimensions:"",supplier:""}]});} }
  ...
  const itemFields=["customer","contract_number","customer_po","product_code","product_name","spec","quantity","pieces","gross_weight","net_weight","pallet_count","volume","order_total_pieces","gross_weight_per_box","net_weight_per_box",...(warehouseUser?["inventory_receipt_number","warehouse_location","inventory_source"]:[]),"loading_factory"];
  const qcByIndex=new Map(qcResults?.map(result=>[result.itemIndex,result])||[]);
  const qcSummaryText=qcResults?qcSummary(task.items,qcResults).text:"";
  ...
  return <div className="content shipment-detail"><Link className="back-button" href={backHref}><ArrowLeft size={15}/>{backLabel}</Link><div className="page-heading shipment-detail-heading"><div><p className="eyebrow">走柜任务 #{task.id}</p><h1 title={taskTitle}>{taskTitle}</h1></div><div className="heading-status-actions"><select className="heading-status-select" aria-label="任务状态" value={task.status} disabled={saving||warehouseUser||task.status!=="PendingReview"} onChange={event=>void update({status:event.target.value})}><option value={task.status}>{shipmentStatus[task.status]}</option>{task.status==="PendingReview"&&<option value="PendingShipment" disabled={!task.plannedShipDate}>{task.plannedShipDate?"待走货":"待走货（请先填写日期）"}</option>}</select>{!warehouseUser&&(task.status==="PendingReview"||task.status==="PendingShipment")&&<button className="ghost-button action-cancel" disabled={saving} onClick={()=>void cancelTask()}>取消任务</button>}<button className="ghost-button action-delete" disabled={saving} onClick={()=>void deleteTask()}><Trash2 size={14}/>删除任务</button></div></div>{error&&<div className="notice">{error}</div>}
    ...
    <section className={`panel shipment-items editable ${warehouseUser?"warehouse-view":"shipping-view"}`}><div className="panel-title"><div><h2>货物明细</h2><p>{warehouseUser?"放货区和入库单号从本地库存表查询，仓务可人工修正":"核对走柜货物资料、数量和装箱信息"}</p></div>{!warehouseUser&&<button className="ghost-button" onClick={addItem}><Plus size={14}/>新增明细</button>}</div><div className="shipment-item-table"><div><b>洋行</b><b>合同号</b><b>客人PO</b><b>货号</b><b>货名</b><b>规格</b><b>数量</b><b>件数</b><b>毛重</b><b>净重</b><b>卡板</b><b>体积</b><b>每单总件数</b><b>每箱毛重</b><b>每箱净重</b>{warehouseUser&&<><b>入库单号</b><b>放货区</b><b>来源表</b></>}<b>做柜工厂</b><b>操作</b><b>客户验货结果/第三方验货结果</b></div>{task.items.map((item,index)=><div key={index}>{itemFields.map(key=>{const derived=key==="customer";const warehouseEditable=key==="quantity"||key==="pieces"||key==="inventory_receipt_number"||key==="warehouse_location";const warehouseLocked=warehouseUser&&(task.status!=="PendingShipment"||!warehouseEditable);const inventoryField=key==="inventory_receipt_number"||key==="warehouse_location";const pendingWeight=key==="gross_weight"||key==="net_weight";const raw=key==="customer"?task.customer:item[key];const placeholder=pendingWeight?"待货品资料":key==="inventory_receipt_number"?"待查询入库单":key==="warehouse_location"?"待查询放货区":key==="inventory_source"?"待查询来源表":"";return key==="spec"||compactNumberFields.has(key)?<EditableItemInput key={key} value={raw} disabled={derived||warehouseLocked} label={`${key}-${index+1}`} placeholder={placeholder} displayValue={value=>key==="spec"?fullSpecification(value):itemDisplayValue(key,value)} onChange={value=>changeItem(index,key,key==="spec"?specificationValue(value):value)}/>:<input key={key} className={inventoryField?"inventory-match-input":""} aria-label={`${key}-${index+1}`} title={key==="inventory_source"?String(item.inventory_source_full||item.inventory_source||""):undefined} disabled={derived||warehouseLocked} placeholder={placeholder} value={itemDisplayValue(key,raw)} onChange={event=>changeItem(index,key,event.target.value)}/>})}{warehouseUser?<span>{task.status==="PendingShipment"?"可保存":"只读"}</span>:<button className="icon-danger" aria-label={`删除第${index+1}条明细`} onClick={()=>removeItem(index)}><Trash2 size={14}/></button>}<span className={`qc-item-result ${qcState(qcByIndex.get(index))}`} title={qcByIndex.get(index)?.latest?`${qcByIndex.get(index)?.latest?.site||"待分配"} · ${String(qcByIndex.get(index)?.latest?.inspectionDate||"").slice(0,10)||"日期未填"} · ${qcResultText(qcByIndex.get(index))}${qcByIndex.get(index)?.latest?.holdRejectReason?` · ${qcByIndex.get(index)?.latest?.holdRejectReason}`:""}`:undefined}>{qcResults?qcResultText(qcByIndex.get(index)):"待查询"}</span></div>)}</div></section>
  </div>;
}
