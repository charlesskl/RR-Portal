...
(
  ...
  SHIPPING_ENV="$PROJECT_DIR/../09-voyageplex/.env.local"
  if [[ -f "$SHIPPING_ENV" ]]; then
    QC_SHIPPING_API_KEY="$(sed -n 's/^QC_SYSTEM_API_TOKEN=//p' "$SHIPPING_ENV" | head -n 1)"
    [[ ${#QC_SHIPPING_API_KEY} -ge 32 ]] && export QC_SHIPPING_API_KEY
  fi
